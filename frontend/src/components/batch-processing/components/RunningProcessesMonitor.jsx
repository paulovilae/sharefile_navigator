    ...
    const fetchRunningProcesses = async () => {
        ...
        try {
            ...
            if (mountedRef.current) {
                ...
                // DIAGNOSTIC: Log what we received from the backend
                console.log('[RunningProcessesMonitor] DIAGNOSTIC: Received response from /api/ocr/batch/list:', response.data);
                console.log('[RunningProcessesMonitor] DIAGNOSTIC: Extracted jobs:', jobs);
                console.log('[RunningProcessesMonitor] DIAGNOSTIC: Number of jobs:', Object.keys(jobs).length);
                
                // DIAGNOSTIC: Log details of each job
                Object.entries(jobs).forEach(([batchId, job]) => {
                    console.log(`[RunningProcessesMonitor] DIAGNOSTIC: Job ${batchId}:`, {
                        status: job.status,
